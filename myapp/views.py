...
def answer(request):
    ...
    if request.method == "POST":
        ...
        for i in range(len(histograms)):
            ...
            if compare(desc, histograms[i]) < 0 or euclideanDistance(desc, histograms[
                i]) > 1:  # if correlation between the two histograms is less than 0.05, paint it black
        ...
        def gen_data(base_image, quantity, label, data, target, shift_value=4):
            for i in range(quantity):
                data.append(shift(base_image, [random.randint(-shift_value, shift_value),
                                               random.randint(-shift_value, shift_value)]))
                target.append(label)
        ...
        for i in range(N):
            for j in range(M - 1, -1, -1):
                ...
                ## big rectangle
                tot2 = np.count_nonzero(copy_matrix[max(0, i - 12 * diam + 1):min(i + 6 * diam + 1, N),
        ...
        plt.figure(figsize=(16, 8))
        plt.imshow(img_ml)
        plt.savefig('media/' + 'upload_result.png')

        context['flag']=1
        context['pic']='upload_result.png'
        print('识别结束！')


    return  render(request, 'index-answer.html',context)
